...
export class SessionManager {
  ...
  armPopupAction(sessionId: string, openerTabId: number): void {
    ...
    if (this.findOtherControllingSession(openerTabId, sessionId)) return;
  ...
  adoptPopupTab(tab: PopupLineageTab): SessionContext | null {
    ...
    for (const [sessionId, pending] of this.pendingPopupActions) {
      ...
      if (
        ...
        !this.findOtherControllingSession(tab.openerTabId, sessionId)
      ) {
  ...
  private findOtherControllingSession(tabId: number, currentSessionId: string): string | null {
