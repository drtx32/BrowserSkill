...
pub fn dispatch(cmd: WikiCmd, format: Format) -> Result<(), CliError> {
    ...
    let (method, params) = match cmd.sub {
        ...
        WikiSub::Current(args) => (
            Method::WikiStatus,
            json!({"session_id": args.session_id, "current": true}),
        ),
    ...
    match format {
        Format::Json | Format::Human => println!(
            "{}",
            serde_json::to_string_pretty(&result).map_err(|e| CliError::Local(e.into()))?
        ),
    }
    Ok(())
}
...
fn scoped(args: &WikiScopeArgs) -> Result<Value, CliError> {
    ...
    let explicit = args.page_instance_id.is_some()
        || args.browser_id.is_some()
        || args.tab_id.is_some()
        || args.document_id.is_some()
        || args.origin.is_some();
    if explicit {
        if args.page_instance_id.is_none()
            || args.browser_id.is_none()
            || args.tab_id.is_none()
            || args.document_id.is_none()
            || args.origin.is_none()
        {
            return Err(CliError::Local(anyhow::anyhow!(
                "explicit Wiki scope requires page_instance_id, browser_id, tab_id, document_id, and origin"
            )));
        }
...
fn scope(args: &WikiScopeArgs) -> WikiScope {
    WikiScope {
        browser_id: args.browser_id.clone().unwrap(),
        session_id: args.session_id.clone(),
        tab_id: args.tab_id.unwrap(),
        document_id: args.document_id.clone().unwrap(),
        origin: args.origin.clone().unwrap(),
    }
}
...
mod tests {
    ...
    fn explicit_scope_is_preserved_for_debug_reads() {
        ...
        .unwrap();
        let WikiSub::Status(args) = parsed.sub else {
