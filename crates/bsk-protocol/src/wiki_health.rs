...
use crate::wiki::{
    Claim, Completeness, RecordStatus, Region, Relation, RelationType, SemanticDelta,
};
...
pub fn compute_health(
    ...
    let eligible_regions: Vec<&Region> = regions
        .iter()
        .filter(|r| region_ids.contains(r.region_id.as_str()))
        .collect();
    let eligible_claims: Vec<&Claim> = claims
        .iter()
        .filter(|c| claim_ids.contains(c.claim_id.as_str()))
        .collect();
    ...
    let orphan_ids: Vec<String> = eligible_regions
        .iter()
        .filter(|r| {
            ...
            let parent = r
                .parent_region_id
                .as_ref()
                .map(|p| region_ids.contains(p.as_str()))
                .unwrap_or(false);
            let linked = relations
                .iter()
                .any(|rel| valid_relation(&rel) && rel.to_id == r.region_id);
            r.status != RecordStatus::Active || !(evidence && (parent || linked))
        })
        .map(|r| r.region_id.clone())
        .collect();
    let stale_ids: Vec<String> = eligible_claims
        .iter()
        .filter(|c| {
            matches!(
                c.status,
                RecordStatus::Stale | RecordStatus::Superseded | RecordStatus::Uncertain
            )
        })
        .map(|c| c.claim_id.clone())
        .collect();
    let eligible_deltas: Vec<&(String, SemanticDelta)> = deltas
        .iter()
        .filter(|(id, _)| eligibility.delta_ids.iter().any(|wanted| wanted == id))
        .collect();
    let complete_ids: Vec<String> = eligible_deltas
        .iter()
        .filter(|(_, d)| d.completeness == Completeness::Complete)
        .map(|(id, _)| (*id).clone())
        .collect();
    let refresh_ids: Vec<String> = eligible_deltas
        .iter()
        .filter(|(_, d)| d.completeness == Completeness::FullRefreshRequired)
        .map(|(id, _)| (*id).clone())
        .collect();
    let eligible_frontiers: Vec<&FrontierSample> = frontiers
        .iter()
        .filter(|f| frontier_ids.contains(f.frontier_id.as_str()))
        .collect();
    let resumable_ids: Vec<String> = eligible_frontiers
        .iter()
        .filter(|f| {
            f.last_complete_revision
                .is_some_and(|r| r <= f.evidence_revision)
        })
        .map(|f| f.frontier_id.clone())
        .collect();
    let duplicate_ids: Vec<String> = eligible_frontiers
        .iter()
        .flat_map(|f| {
            let unique = f.item_ids.iter().collect::<BTreeSet<_>>().len();
            if unique < f.item_ids.len() {
                Some(f.frontier_id.clone())
            } else {
                None
            }
        })
